...
const navLinks: StaffNavItem[] = [
    { label: 'My Tasks',    href: '/staff',             icon: CheckSquare },
    { label: 'My Projects', href: '/staff/projects',    icon: FolderKanban, fuzzy: true },
    { label: 'Tickets',     href: '/admin/tickets',     icon: Ticket,     fuzzy: true, badgeKey: 'tickets' },
    { label: 'Inbox',       href: '/staff/inbox',       icon: MessageSquare,            badgeKey: 'messages' },
    { label: 'Settings',    href: '/staff/settings',    icon: Settings,   fuzzy: true },
];
...
function NavLink({
    ...
    const linkClasses = `
        ...
        ${isActive
            ? 'bg-[#D4AF37] text-[#0A1128] shadow-lg shadow-[#D4AF37]/20 font-semibold'
            : collapsed
...
export default function StaffLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex">
            ...
            <aside
                className={`
                    bg-[#0A1128] text-white border-r border-[#D4AF37]/10
            ...
            >
                {/* ── Header ── */}
                <div className={`border-b border-[#D4AF37]/10 ${collapsed && !isMobile ? 'p-3' : 'p-6'}`}>
                    <div className={`flex items-center ${collapsed && !isMobile ? 'flex-col gap-3' : 'justify-between'}`}>
                        {!collapsed || isMobile ? (
                            <div className="min-w-0">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-full bg-[#D4AF37]/20 border border-[#D4AF37]/30 flex items-center justify-center overflow-hidden shrink-0">
                                        {profile?.avatarUrl ? (
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-[#D4AF37]">Team Member</p>
                                    </div>
                                </div>
                            </div>
                        ) : (
                            <div className="flex flex-col items-center gap-2">
                                <div className="w-9 h-9 rounded-full bg-[#D4AF37]/20 border border-[#D4AF37]/30 flex items-center justify-center overflow-hidden">
                                    {profile?.avatarUrl ? (
                                        <img src={profile.avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
                                    ) : (
                                        <span className="text-[#D4AF37] font-bold text-sm">{user?.email?.[0].toUpperCase()}</span>
                                    )}
                                </div>
                            </div>
                        )}
                ...
                <nav className={`flex-1 overflow-y-auto ${collapsed && !isMobile ? 'p-2 space-y-1' : 'p-4 space-y-1'}`}>
                    ...
                    {/* ── Quick Create ── */}
                    <div className={`border-t border-[#D4AF37]/10 pt-3 mt-3 ${collapsed && !isMobile ? 'px-0' : ''}`}>
                        <div className="relative">
                            <button
                                ...
                                className={`
                                    ...
                                    ${collapsed && !isMobile
                                        ? 'justify-center px-0 py-2 text-slate-400 hover:text-[#D4AF37] hover:bg-white/5'
                                        : 'px-3 py-2.5 text-slate-400 hover:text-white hover:bg-white/5 border border-dashed border-[#D4AF37]/20 hover:border-[#D4AF37]/40'
                                    }
                                `}
                            ...
                            {quickCreateOpen && (
                                <>
                                    ...
                                    >
                                        ...
                                        >
                                            <div className="w-8 h-8 rounded-lg bg-[#D4AF37]/10 text-[#D4AF37] flex items-center justify-center transition-transform group-hover:scale-110">
                ...
                {/* ── User section ── */}
                <div className={`border-t border-[#D4AF37]/10 ${collapsed && !isMobile ? 'p-3' : 'p-4'}`}>
                    <div className={`flex items-center ${collapsed && !isMobile ? 'justify-center' : 'space-x-3'} mb-3 px-2`}>
                        <div className="w-8 h-8 rounded-full bg-[#D4AF37]/20 flex items-center justify-center text-[#D4AF37] font-bold overflow-hidden border border-[#D4AF37]/30 shrink-0">
                            {profile?.avatarUrl ? (
                        ...
                        {(!collapsed || isMobile) && (
                            <div className="overflow-hidden min-w-0">
                                ...
                                <p className="text-[#D4AF37] text-xs">Team Member</p>
                            </div>
                        )}
                    </div>
                    <button
                ...
                {isMobile && (
                    <div className="flex items-center justify-between mb-6">
                        <button
                            ...
                            className="p-2 -ml-2 text-slate-600 hover:text-[#0A1128] transition-colors"
