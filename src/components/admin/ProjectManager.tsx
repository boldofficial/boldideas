      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 relative z-10">
        <div>
            ...
            <p className="text-[10px] font-mono text-slate-400 mt-1 uppercase tracking-widest">
                Read / Write Access // Table: Projects
            </p>
        </div>
        <button 
           onClick={() => { setCurrentProject({}); setIsEditing(true); }}
           className="bg-brand-navy text-white px-6 py-3 rounded-sm text-xs font-bold uppercase tracking-widest hover:bg-brand-gold hover:text-brand-navy transition-all flex items-center border border-transparent hover:border-brand-navy/20 shadow-lg"
        >
            ...
            <div key={project.id} className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 bg-white border border-slate-200 hover:border-brand-gold transition-all group shadow-sm hover:shadow-md">
                <div className="mb-4 md:mb-0">
                    <div className="flex items-center gap-3 mb-1">
                        <h4 className="font-black text-brand-navy uppercase tracking-tight">{project.title}</h4>
                    ...
                    <p className="text-xs text-slate-500 font-mono truncate max-w-lg opacity-70">
                         &gt; {project.problem}
                    </p>
                </div>
          ...
          <div className="fixed inset-0 bg-brand-navy/90 backdrop-blur-md flex items-center justify-center z-[9999] p-4 animate-in fade-in duration-200">
              <div className="bg-slate-50 rounded-sm p-0 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-brand-gold relative">
                  ...
                  <div className="bg-brand-navy p-6 flex justify-between items-center sticky top-0 z-10 border-b border-brand-gold/20">
                      <div>
                          <h3 className="text-lg font-black text-white uppercase tracking-wider">{currentProject.id ? 'Edit_Protocol' : 'Init_New_Protocol'}</h3>
                          <p className="text-[10px] font-mono text-brand-gold/60">SECURE_FORM_UPLINK</p>
                      </div>
                  ...
                  <form onSubmit={handleSave} className="p-8 space-y-6">
                      <div>
                          <label className="block text-[10px] font-bold uppercase text-brand-navy/60 tracking-widest mb-2 font-mono">Title (Client Name)</label>
                          <input 
                            className="w-full p-3 bg-white border border-slate-200 focus:border-brand-gold focus:ring-1 focus:ring-brand-gold/50 outline-none rounded-none text-sm font-bold text-brand-navy placeholder:text-slate-300" 
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-[10px] font-bold uppercase text-brand-navy/60 tracking-widest mb-2 font-mono">Tags (Comma Separated)</label>
                            <input 
                                className="w-full p-3 bg-white border border-slate-200 focus:border-brand-gold focus:ring-1 focus:ring-brand-gold/50 outline-none rounded-none text-sm font-mono text-brand-navy placeholder:text-slate-300"
                        ...
                        <div>
                            <label className="block text-[10px] font-bold uppercase text-brand-navy/60 tracking-widest mb-2 font-mono">Image URL</label>
                            <input 
                                className="w-full p-3 bg-white border border-slate-200 focus:border-brand-gold focus:ring-1 focus:ring-brand-gold/50 outline-none rounded-none text-sm font-mono text-brand-navy placeholder:text-slate-300"
                      ...
                      <div className="space-y-4">
                          <div className="p-4 bg-white border border-slate-100 relative group focus-within:border-red-400/50 transition-colors">
                              <label className="block text-[10px] font-bold uppercase text-red-500 tracking-widest mb-2 font-mono flex items-center">
                                  <span className="w-1.5 h-1.5 bg-red-500 mr-2 rounded-full"></span> 
                                  Target Anomaly (The Problem)
                              </label>
                              <textarea 
                          ...
                          <div className="p-4 bg-white border border-slate-100 relative group focus-within:border-blue-400/50 transition-colors">
                              <label className="block text-[10px] font-bold uppercase text-blue-500 tracking-widest mb-2 font-mono flex items-center">
                                  <span className="w-1.5 h-1.5 bg-blue-500 mr-2 rounded-full"></span>
                                  Process Deployed (The Solution)
                              </label>
                              <textarea 
                                className="w-full p-0 bg-transparent border-none focus:ring-0 text-sm text-slate-600 leading-relaxed resize-none h-20" 
                          ...
                          <div className="p-4 bg-white border border-slate-100 relative group focus-within:border-emerald-400/50 transition-colors">
                              <label className="block text-[10px] font-bold uppercase text-emerald-500 tracking-widest mb-2 font-mono flex items-center">
                                  <span className="w-1.5 h-1.5 bg-emerald-500 mr-2 rounded-full"></span>
                                  Operational Outcome (The Result)
                              </label>
                              <textarea 
                                className="w-full p-0 bg-transparent border-none focus:ring-0 text-sm text-slate-600 leading-relaxed resize-none h-20" 
                      ...
                      <div className="flex gap-4 pt-4 sticky bottom-0 bg-slate-50 pb-2 border-t border-slate-100">
                          <button 
                            ...
                            className="flex-1 px-6 py-4 bg-slate-200 text-slate-500 font-bold uppercase tracking-widest text-xs hover:bg-slate-300 transition-colors rounded-none"
                          >
                            Cancel_Op
                          </button>
                          <button 
                            type="submit" 
                            className="flex-[2] bg-brand-gold text-brand-navy font-black py-4 uppercase tracking-widest hover:bg-brand-navy hover:text-white transition-all text-xs rounded-none shadow-lg"
                          >
                            Save_Record
                          </button>
                      </div>
