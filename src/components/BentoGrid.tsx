...
const BentoGrid: React.FC = () => {
  ...
  const stats = {
    strategy: {
      value: "Roadmap",
      label: "AI Strategy",
      detail: "A tailored roadmap that prioritizes high-impact AI use cases for your business.",
      id: "STRAT_01"
    },
    automation: {
      value: "Seamless",
      label: "Workflow",
      detail: "We identify repetitive tasks and build automations to eliminate manual errors and scale.",
      id: "AUTO_02"
    },
    seo: {
      value: "1000s",
      label: "Content Pages",
      detail: "Scalable content engines that generate thousands of targeted, AI-optimized landing pages.",
      id: "SEO_03"
    }
  };
  ...
  const services = [
    ...
    },
    {
      ...
      description: 'We eliminate manual data entry and repetitive tasks by connecting your apps (e.g. OpenAI, Zapier, Make).',
      icon: '⚡',
      status: 'ACTIVE',
      position: 'right',
      highlight: true,
    ...
    },
    {
      ...
      description: 'We optimize your ad spend using AI algorithms to target the right audience at the right time.',
      icon: '📈',
      status: 'ADS',
      position: 'right',
    },
    {
      ...
      description: 'We deploy intelligent customer service agents that work 24/7 to qualify leads and support users.',
      icon: '🤖',
      status: 'AGENT',
      position: 'left',
    },
    {
      ...
      description: 'We turn raw data into actionable dashboards so you can make decisions based on facts, not guesses.',
      icon: '📊',
      status: 'DATA',
      position: 'right',
    }
  ...
  return (
    <section id="services" className="py-24 px-4 bg-brand-light relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-24 mb-32 items-end">
          ...
          <div className="bg-white/50 backdrop-blur-sm border border-brand-navy/10 p-1 rounded-sm shadow-2xl relative">
            ...
            <div className="bg-brand-navy/5 p-2 flex justify-between items-center border-b border-brand-navy/5 mb-6">
                ...
                <span className="font-mono text-[8px] text-brand-navy/40 uppercase tracking-widest">SYS_MONITOR_V2.0</span>
            </div>
            ...
            <div className="px-6 pb-6">
                ...
                {(['strategy', 'automation', 'seo'] as const).map((tab) => (
                    ...
                    >
                        {tab === 'strategy' ? 'STRATEGY_MOD' : tab === 'automation' ? 'AUTO_MOD' : 'SEO_MOD'}
                    </button>
                ))}
                </div>

                <div className="min-h-[120px] animate-fade-in flex flex-col justify-center relative">
                    <div className="absolute top-0 right-0 font-mono text-[9px] text-brand-gold/60">{stats[activeTab].id}</div>
        ...
        <div className="relative pt-12 pb-24">
          ...
          <div className="flex flex-col space-y-24 lg:space-y-32 relative z-10">
            {services.map((node) => (
              <div key={node.id} className={`flex flex-col lg:flex-row items-center w-full group ${node.position === 'right' ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
                {/* Content Side */}
                <div className={`w-full lg:w-[45%] ${node.position === 'right' ? 'text-left' : 'text-right'}`}>
                  ...
                  <p className={`text-sm text-slate-500 font-light max-w-sm font-mono leading-relaxed ${node.position === 'right' ? '' : 'ml-auto'}`}>
                    {node.description}
                  </p>
                </div>
                ...
                <div className="relative w-full lg:w-[10%] flex justify-center py-8 lg:py-0">
                  <div className={`absolute ${node.position === 'right' ? 'right-1/2 w-[500px]' : 'left-1/2 w-[500px]'} top-1/2 -translate-y-1/2 h-[1px] bg-brand-navy/10 hidden lg:block`}></div>
                  
                  {/* Node Connector Point */}
                   <div className={`relative w-16 h-16 border border-brand-navy/10 flex items-center justify-center transition-all duration-500 shadow-xl z-20 ${
                    node.highlight ? 'bg-brand-navy scale-110' : 'bg-white hover:scale-110'
                  } ${node.highlight ? 'rounded-sm' : 'rounded-sm rotate-45 group-hover:rotate-0'}`}>
                    
                    <div className={node.highlight ? '' : '-rotate-45 group-hover:rotate-0 transition-transform duration-500'}>
                        <span className="text-2xl">{node.icon}</span>
                    </div>

                    {/* Technical Markers */}
                    <div className={`absolute -top-1 -right-1 w-2 h-2 border-t border-r ${node.highlight ? 'border-brand-gold' : 'border-brand-navy/20'}`}></div>
                    <div className={`absolute -bottom-1 -left-1 w-2 h-2 border-b border-l ${node.highlight ? 'border-brand-gold' : 'border-brand-navy/20'}`}></div>
                  </div>
                </div>
