        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          ...
          <div className="order-2 lg:order-1 relative">
            ...
            <div className="inline-block px-3 py-1 mb-8 border border-brand-navy/10 bg-brand-light">
              <span className="text-[10px] md:text-xs font-mono font-black uppercase tracking-[0.3em] text-brand-navy">Mission_Log: 01</span>
            </div>
            ...
            <div className="relative group">
               ...
               <div className="bg-white p-8 relative border border-brand-navy/10 shadow-xl rounded-sm">
                  ...
                  <div className="flex justify-between items-center mb-4 border-b border-brand-navy/5 pb-2">
                     <span className="font-mono text-[9px] md:text-[10px] text-brand-gold uppercase tracking-widest">TARGET_AUDIENCE</span>
          ...
          <div className="order-1 lg:order-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               ...
               <div className="md:col-span-2 bg-brand-navy p-8 text-white relative overflow-hidden group border border-brand-navy rounded-sm">
                  <div className="absolute top-0 right-0 p-4 font-mono text-[9px] md:text-[10px] text-white/20 tracking-widest">CORE_VALUES</div>
                  <div className="absolute bottom-0 right-0 w-32 h-32 bg-brand-gold/10 blur-[50px] rounded-full"></div>
                  
                  <span className="text-brand-gold font-mono text-xs md:text-sm mb-2 block">&gt; SYSTEM_VALUES</span>
               ...
               {[
                 ...
                 <div key={i} className="bg-white p-6 border border-brand-navy/10 hover:border-brand-gold hover:shadow-lg transition-all duration-300 group rounded-sm relative">
                   ...
                   <div className="font-mono text-[9px] md:text-[10px] text-slate-300 mb-4 group-hover:text-brand-gold transition-colors">VAL_{item.id}</div>
                   
                   <div className="mb-3 text-2xl grayscale group-hover:grayscale-0 transition-all">{item.icon}</div>
                   
                   <h4 className="text-sm md:text-base font-black text-brand-navy uppercase tracking-wide mb-2">{item.label}</h4>
                   <p className="text-xs md:text-sm text-slate-500 font-mono leading-relaxed group-hover:text-brand-navy transition-colors">{item.text}</p>
                 </div>
               ))}
            </div>
          </div>
