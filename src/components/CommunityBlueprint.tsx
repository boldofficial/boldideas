...
const CommunityBlueprint: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'audit' | 'build' | 'iter'>('audit');

  const stats = {
    audit: {
      value: "Deep",
      label: "Audit",
      detail: "We audit your current workflows, data, and marketing to find high-impact AI opportunities.",
      id: "PHASE_01"
    },
    build: {
      value: "Launch",
      label: "Tech Stack",
      detail: "We implement the tech stack, set up automations, and launch AI-driven campaigns.",
      id: "PHASE_02"
    },
    iter: {
      value: "10x",
      label: "Iteration",
      detail: "We track impact, learn from the data, and keep improving your growth engine.",
      id: "PHASE_03"
    }
  };

  const steps = [
    { id: '01', title: 'Discover', description: 'We audit your current workflow, apps, and marketing to find high-impact opportunities.', status: 'AUDIT', icon: '🔍' },
    { id: '02', title: 'Design', description: 'We propose custom automation systems and AI strategies tailored to your specific goals.', status: 'PLAN', icon: '📐', highlight: true },
    { id: '03', title: 'Build', description: 'We implement the tech stack, set up automations, and launch AI-ad campaigns.', status: 'BUILD', icon: '🏗️' },
    { id: '04', title: 'Train', description: 'We empower your team with the knowledge they need to run tools effectively.', status: 'TRAIN', icon: '🧠' },
  ];

  return (
    <section id="process" className="py-24 px-4 bg-white relative overflow-hidden border-t border-brand-navy/5">
        ...
        <div className="grid lg:grid-cols-5 gap-12 mb-20 items-center">
          <div className="lg:col-span-3 animate-fade-in relative">
            ...
            <p className="text-slate-500 mb-8 max-w-sm text-lg font-light font-mono">
              &gt; Simple systems, clear playbooks, and campaigns that compound over time.
            </p>
          </div>
          ...
          <div className="lg:col-span-2 bg-white border border-brand-navy/10 p-6 shadow-2xl relative rounded-sm">
            ...
            <div className="flex space-x-6 mb-8 border-b border-brand-navy/5 pb-4 overflow-x-auto no-scrollbar">
              {(['audit', 'build', 'iter'] as const).map((tab) => (
                ...
                >
                  {tab === 'audit' ? 'Step 01' : tab === 'build' ? 'Step 03' : 'Step 05'}
                </button>
              ))}
            </div>
        ...
        <div className="relative">
          ...
          <div className="grid md:grid-cols-2 gap-x-12 gap-y-16 lg:gap-y-24">
            {steps.map((node, index) => (
              <div key={node.id} className={`relative group ${index % 2 !== 0 ? 'md:mt-20' : ''}`}>
                 {/* Connection Dot */}
                 <div className={`absolute top-8 ${index % 2 === 0 ? 'right-0 translate-x-[calc(50%+24px)]' : 'left-0 -translate-x-[calc(50%+24px)]'} w-4 h-4 rounded-full border-2 border-brand-navy bg-white hidden lg:block z-20`}>
                     <div className="absolute inset-1 bg-brand-navy rounded-full"></div>
                 </div>
                 
                 {/* Connector Line to Center */}
                 <div className={`absolute top-10 ${index % 2 === 0 ? 'right-0 -mr-6 w-[calc(50%+24px)]' : 'left-0 -ml-6 w-[calc(50%+24px)]'} h-px bg-brand-navy/10 hidden lg:block`}></div>

                <div className={`p-8 border transition-all duration-300 relative ${
                  node.highlight 
                    ? 'bg-brand-navy border-brand-navy text-white shadow-2xl' 
                    : 'bg-white border-brand-navy/10 hover:border-brand-gold'
                } rounded-sm`}>
                   {/* Tech Corners */}
                   <div className={`absolute top-0 right-0 w-3 h-3 border-t border-r ${node.highlight ? 'border-brand-gold' : 'border-brand-navy/20'}`}></div>
                  ...
                  <div className="flex items-center space-x-5 mb-4">
                    ...
                    <h3 className={`text-xl font-black tracking-tight ${node.highlight ? 'text-white' : 'text-brand-navy'}`}>{node.title}</h3>
                  </div>
                  <p className={`text-sm font-light leading-relaxed font-mono ${node.highlight ? 'text-white/70' : 'text-slate-500'}`}>{node.description}</p>
                </div>
              </div>
            ))}
          </div>
